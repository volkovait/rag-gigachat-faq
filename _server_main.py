...
def _load_env_simple(env_path: Path) -> None:
    """Minimal KEY=VALUE .env loader if python-dotenv is not installed."""
...
try:
    from dotenv import load_dotenv as _load_dotenv_pkg

    _load_dotenv_pkg()
except ImportError:
    _load_env_simple(Path(__file__).resolve().parent / ".env")

if sys.version_info < (3, 8):
    sys.stderr.write("Python 3.8 or higher is required: python3 server.py\n")
    sys.exit(1)
...
try:
    ...
    sys.stderr.write(
        "Не хватает зависимостей (часто это httpx). Ошибка импорта: %s\n\n"
        "Если `python3 -m pip install -r requirements.txt` пишет "
        "\"externally-managed-environment\" (типично для Python из Homebrew),\n"
        "ставьте пакеты в виртуальное окружение:\n\n"
        "  python3 -m venv .venv\n"
        "  source .venv/bin/activate\n"
        "  pip install -r requirements.txt\n"
        "  python server.py\n\n"
        "Не используйте голый `pip`/`python`, если это Python 2.7.\n"
        % (_imp_err,)
    )
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _post_index(self) -> None:
        ...
        if len(pieces) != len(vectors):
            json_response(self, 502, {"error": "embedding count mismatch"})
            return
        CHUNKS = [{"text": t, "embedding": v} for t, v in zip(pieces, vectors)]
        PREPARED.clear()
